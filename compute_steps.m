...
function [commands] = compute_steps()
    ...
    while(current_u < MAX_U + 0.1)
        [next_u, V_l_command, V_r_command] = neeto_step(current_u, DELTA_T, V_l_symbolic, V_r_symbolic, sub_eval_symb, DU_DT_SCAN, DU_DT_SCAN_MAX, MAX_NEETO_SPEED);
        commands = [commands; V_l_command V_r_command DELTA_T * size(commands, 1)];
        current_u = next_u
    end
end
